...
public class CertificateOfDeposit : BankAccount
{
  ...
  public CertificateOfDeposit(
    ...
    double balance = 1000,
    int termInMonths = 6,
    double earlyWithdrawalPenalty = 0.1
  )
    : base(customer, balance, "Certificate of Deposit")
  {
  ...
  public override string DisplayAccountInfo()
  {
    return base.DisplayAccountInfo()
      + $", Maturity Date: {MaturityDate.ToShortDateString()}, Early Withdrawal Penalty: {EarlyWithdrawalPenalty * 100}%, Interest Rate: {InterestRate * 100}%";
  }
}
